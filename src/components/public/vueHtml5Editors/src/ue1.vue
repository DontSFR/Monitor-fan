...
class LoadEvent {
  ...
  on (eventName, callback) {
    this.listeners[eventName] = this.listeners[eventName] === undefined ? [] : ''
...
export default {
  ...
  data () {
    return {
      ...
      history: '',
      defaultConfig: {
        UEDITOR_HOME_URL: process.env.NODE_ENV === 'production' ? '/center/static/UEditor/' : '/static/UEditor/',
    ...
    _initEditor (value) {
      this.$nextTick(() => {
        ...
        this.editor.addListener('ready', () => {
          ...
          if (this.history !== '') {
            this.editor.setContent(this.history)
          }
  ...
  watch: {
    value: {
      handler: function (value) {
        // console.log(3, value, this.editor.getContent)
